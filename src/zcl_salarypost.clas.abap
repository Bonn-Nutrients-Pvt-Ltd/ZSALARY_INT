CLASS zcl_salarypost DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_salarypost IMPLEMENTATION.
  ...
  METHOD postData.
    ...
    TRY.
        ...
        LOOP AT tt_json_structure INTO DATA(wa).
          ...
          SELECT SINGLE * FROM zr_salary
          WHERE Plant = @wa-plant AND DueDate = @wa-DueDate
                AND  EmployeeCode = @wa-employeecode
             INTO @DATA(wa_data).

          IF wa_data-Isvalidate = abap_false.
            message = |Data is not validated for Employee Code: { wa_data-EmployeeCode }| .
...
*                               GROSS SALARY
                          ( glaccountlineitem = |001|
                            ...
                            DocumentItemText = |Salary for m/o { get_month_year( dcDate ) } ({ lv_bpName })|
                            AssignmentReference = wa_data-EmployeeCode
                            BusinessPlace = wa_data-Plant
                            _currencyamount = VALUE #( (
                                            currencyrole = '00'
                                            journalentryitemamount = wa_data-GrossSalary * -1
                                            currency = 'INR' ) ) ) )
    _glitems = VALUE #(
                        ...
                        _currencyamount = VALUE #( (
                                            currencyrole = '00'
                                            journalentryitemamount = wa_data-GrossSalary
                                            currency = 'INR' ) ) ) )
    ).
...
*                               Advance
    ...
    ELSE.
      ...
      IF document IS NOT INITIAL.
        message = |Document Created Successfully: { document } & { document2 }|.
        MODIFY ENTITIES OF zr_salary
        ENTITY ZrSalary
        UPDATE FIELDS ( Accountingdocument Accountingdocument2 Isposted Errorlog )
        WITH VALUE #(  (
